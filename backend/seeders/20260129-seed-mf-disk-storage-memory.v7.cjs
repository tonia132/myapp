...
function typeIsNumeric(colDef) {
  const t = String(colDef?.type ?? "").toLowerCase();
  return /(bigint|int|decimal|numeric|float|double)/.test(t);
}

function typeIsString(colDef) {
  const t = String(colDef?.type ?? "").toLowerCase();
  return /(char|text|enum|set|uuid)/.test(t);
}

async function resolveFirstIdFromAnyTable(queryInterface, transaction, tableCandidates) {
  for (const t of tableCandidates) {
    try {
      const desc = await queryInterface.describeTable(t, { transaction });
      const pk = findPrimaryKey(desc);
      if (!pk) continue;

      const rows = await queryInterface.sequelize.query(
        `SELECT \`${pk}\` AS id FROM \`${t}\` ORDER BY \`${pk}\` ASC LIMIT 1`,
        { type: QueryTypes.SELECT, transaction }
      );
      if (rows && rows[0] && rows[0].id != null) return rows[0].id;
    } catch (e) {
      // ignore
    }
  }
...
async function resolveSeedUserValue(queryInterface, transaction, colDef) {
  // numeric -> pick an existing user id if possible
  if (typeIsNumeric(colDef)) {
    const id = await resolveFirstIdFromAnyTable(queryInterface, transaction, [
      'users',
      'Users',
      'user',
      'User',
  ...
  // string/uuid -> safe system marker
  if (typeIsString(colDef)) return 'system';
...
module.exports = {
  async up(queryInterface) {
    ...
    const setColTestCases = firstExisting(setCols, ['testCases', 'test_cases', 'caseCodes', 'cases']);
      const setColCreatedBy = firstExisting(setCols, ['createdBy', 'created_by', 'createdById', 'created_by_id']);
      const setColUpdatedBy = firstExisting(setCols, ['updatedBy', 'updated_by', 'updatedById', 'updated_by_id']);
      let seedUserValue = null;
      if (setColCreatedBy || setColUpdatedBy) {
        const colDef = setDesc[setColCreatedBy || setColUpdatedBy];
        seedUserValue = await resolveSeedUserValue(queryInterface, transaction, colDef);
      }
      ...
      const itemColCreatedBy = firstExisting(itemCols, ['createdBy', 'created_by', 'createdById', 'created_by_id']);
      const itemColUpdatedBy = firstExisting(itemCols, ['updatedBy', 'updated_by', 'updatedById', 'updated_by_id']);
      let seedUserValueForItems = seedUserValue;
      if ((itemColCreatedBy || itemColUpdatedBy) && seedUserValueForItems == null) {
        const colDef = itemDesc[itemColCreatedBy || itemColUpdatedBy];
        seedUserValueForItems = await resolveSeedUserValue(queryInterface, transaction, colDef);
      }
      ...
      for (const s of DEFAULT_TEST_SETS) {
        ...
        if (existing && existing[0] && existing[0].id != null) {
          ...
          if (setColTestCases && setCols.has(setColTestCases)) patch[setColTestCases] = JSON.stringify(s.codes);
          if (seedUserValue !== null && setColUpdatedBy && setCols.has(setColUpdatedBy)) patch[setColUpdatedBy] = seedUserValue;
          applySoftDeleteDefaults(patch, setCols, setColIsDeleted, setColDeletedAt);
          applyTimestamps(patch, null, setColUpdatedAt, now);
          ...
          if (setColTestCases && setCols.has(setColTestCases)) row[setColTestCases] = JSON.stringify(s.codes);
          if (seedUserValue !== null && setColCreatedBy && setCols.has(setColCreatedBy)) row[setColCreatedBy] = seedUserValue;
          if (seedUserValue !== null && setColUpdatedBy && setCols.has(setColUpdatedBy)) row[setColUpdatedBy] = seedUserValue;
      ...
      for (const s of DEFAULT_TEST_SETS) {
        ...
        for (let i = 0; i < s.codes.length; i++) {
          ...
          if (seedUserValueForItems != null) {
            if (itemColCreatedBy) row[itemColCreatedBy] = seedUserValueForItems;
            if (itemColUpdatedBy) row[itemColUpdatedBy] = seedUserValueForItems;
          }
  ...
  async down(queryInterface) {
    ...
    try {
      ...
      for (const s of DEFAULT_TEST_SETS) {
        await queryInterface.bulkDelete(tables.defaultSets, { [setColName]: s.name }, { transaction });
      }
