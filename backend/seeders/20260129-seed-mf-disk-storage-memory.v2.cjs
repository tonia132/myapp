...
// code -> (category/section) derived from the v0006-style report sheets
const CODE_META = {
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // When DefaultTestSetItem does NOT have a testCaseId FK, we store embedded fields.
    ...
    const itemColCode2 = firstExisting(itemCols, ['code', 'caseCode', 'tcCode', 'testCode']);
    const itemColTestCase = firstExisting(itemCols, ['testCase', 'title', 'name', 'testName']);
    const itemColProcedure2 = firstExisting(itemCols, ['testProcedure', 'procedure', 'steps', 'testSteps', 'method', 'content', 'description', 'desc']);
    const itemColCriteria2 = firstExisting(itemCols, ['testCriteria', 'criteria', 'expected', 'expectedResult', 'passCriteria', 'acceptanceCriteria']);
      ...
      for (const set of DEFAULT_TEST_SETS) {
        ...
        for (let i = 0; i < set.codes.length; i++) {
          ...
          if (itemColCaseId) {
            ...
            if (itemColCategory) row[itemColCategory] = meta.category || tc.sourceSheet || '';
            if (itemColSection) row[itemColSection] = meta.section || '';

            if (itemColCode2) row[itemColCode2] = tc.code || code;
            if (itemColTestCase) row[itemColTestCase] = tc.title || '';
            if (itemColProcedure2) row[itemColProcedure2] = tc.procedure || '';
            if (itemColCriteria2) row[itemColCriteria2] = tc.criteria || '';
            if (itemColEstHours) row[itemColEstHours] = Number.isFinite(tc.estHours) ? tc.estHours : 0;
            if (itemColIsPlanned) row[itemColIsPlanned] = tc.isPlanned === true;
          }
  ...
  async down(queryInterface, Sequelize) {
    ...
    try {
      ...
      const itemColSetId = firstExisting(itemCols, ['defaultTestSetId', 'default_test_set_id', 'setId', 'testSetId', 'test_set_id']);
