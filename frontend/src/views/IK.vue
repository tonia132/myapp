  ...
  <div class="page ik-page-vivid">
    <section class="hero-card">
      <div class="hero-main">
        <div class="hero-left">
          ...
          <div class="hero-copy">
            <div class="hero-eyebrow">{{ text('ikLab.eyebrow', 'Impact Test Scheduling') }}</div>
            <h2 class="hero-title">{{ text('ikLab.title', 'IK 實驗室排程') }}</h2>
            <div class="hero-subtitle">
              {{ text('ikLab.subtitle', '快速查看每日排程、建立時段並追蹤目前實驗室使用狀態') }}
            </div>
          </div>
        </div>
      ...
      <div class="stats-grid">
        <div class="stat-card stat-primary">
          <div class="stat-label">{{ text('ikLab.stats.total', '排程筆數') }}</div>
          <div class="stat-value">{{ schedulesSorted.length }}</div>
        </div>

        <div class="stat-card">
          <div class="stat-label">{{ text('ikLab.stats.totalHours', '總時數') }}</div>
          <div class="stat-value">{{ totalHoursText }}</div>
        </div>

        <div class="stat-card">
          <div class="stat-label">{{ text('ikLab.stats.approved', '已核准') }}</div>
          <div class="stat-value">{{ approvedCount }}</div>
        </div>

        <div class="stat-card">
          <div class="stat-label">{{ text('ikLab.stats.pending', '待處理') }}</div>
    ...
    >
      <div class="alert-row">
        <div class="alert-text">
          {{ text('ikLab.filteredAlert', '目前顯示 {date} 的排程資料', { date: selectedDate }) }}
        </div>
        ...
        <div class="alert-meta" v-if="!loading">
          ...
          <el-tag effect="plain" class="pill mini">
            {{ text('ikLab.table.time', '時段') }}：{{ totalHoursText }}
          </el-tag>
        </div>
      </div>
    </el-alert>
    ...
    <el-card shadow="never" class="list-card">
        ...
        <div class="section-head">
          <div>
            <div class="section-title">
              {{
                selectedDate
                  ? text('ikLab.cardTitleWithDate', '{date} 排程清單', { date: selectedDate })
                  : text('ikLab.cardTitleNoDate', '排程清單')
              }}
            </div>
            <div class="section-subtitle">
              {{ text('ikLab.sectionSubtitle', '支援桌面表格與手機卡片顯示，排程資訊更容易閱讀') }}
            </div>
          </div>

          <el-tag effect="plain" round>
            {{ text('ikLab.totalTag', '共 {count} 筆', { count: schedulesSorted.length }) }}
          </el-tag>
        </div>
      </template>
      ...
      <el-empty
        ...
        :description="text('ikLab.emptyDescription', '目前沒有排程資料')"
      />
      ...
      <el-table
        ...
        :loading="loading"
      >
        <el-table-column type="index" :label="text('ikLab.table.index', '#')" width="60" />
        <el-table-column prop="date" :label="text('ikLab.table.date', '日期')" width="120" />

        <el-table-column :label="text('ikLab.table.time', '時段')" width="190">
        ...
        <el-table-column prop="purpose" :label="text('ikLab.table.purpose', '用途')" min-width="240" show-overflow-tooltip>
          <template #default="{ row }">
            <span class="cell-strong">{{ row.purpose }}</span>
          </template>
        </el-table-column>

        <el-table-column :label="text('ikLab.table.remark', '備註')" min-width="200">
          <template #default="{ row }">
        ...
        <el-table-column prop="createdAt" :label="text('ikLab.table.createdAt', '建立時間')" width="170">
          <template #default="{ row }">
            <span class="muted">{{ formatDateTime(row.createdAt) }}</span>
          </template>
        </el-table-column>

        <el-table-column prop="status" :label="text('ikLab.table.status', '狀態')" width="120" align="center">
          <template #default="{ row }">
          ...
          <div class="cards">
            ...
            >
              ...
              <div class="mobile-meta-grid">
                <div class="meta-box">
                  <div class="meta-label">{{ text('ikLab.table.date', '日期') }}</div>
                  <div class="meta-value">{{ row.date || '—' }}</div>
                </div>

                <div class="meta-box">
                  <div class="meta-label">{{ text('ikLab.table.createdAt', '建立時間') }}</div>
                  <div class="meta-value">{{ formatDateTime(row.createdAt) || '—' }}</div>
                </div>
              </div>

              <div class="remark">
                <div class="muted label">{{ text('ikLab.table.remark', '備註') }}</div>
    ...
    <el-dialog
      ...
      :title="text('ikLab.dialogTitle', '新增 IK 排程')"
    ...
    >
      <div class="dialog-hero">
        <div class="dialog-main">
          <div class="dialog-title">{{ text('ikLab.dialogHeroTitle', '建立新的實驗室排程') }}</div>
          <div class="dialog-subtitle">
            {{ text('ikLab.dialogHeroSubtitle', '先選日期與時段，再填入用途與備註，送出後會立即更新清單') }}
          </div>
        </div>

        <div class="dialog-preview" v-if="form.date && form.startTime && form.endTime">
          <div class="preview-label">{{ text('ikLab.preview', '排程預覽') }}</div>
      ...
      >
        <el-form-item :label="text('ikLab.fieldDate', '日期')">
          <el-date-picker
            v-model="form.date"
            type="date"
            value-format="YYYY-MM-DD"
        ...
        <el-form-item :label="text('ikLab.fieldTimeRange', '時段')">
          <div class="time-range">
            <el-time-select
              ...
              :placeholder="text('ikLab.timeStartPlaceholder', '開始時間')"
              class="ctrl w-time"
            />
            <span class="sep">~</span>
            <el-time-select
              ...
              :placeholder="text('ikLab.timeEndPlaceholder', '結束時間')"
              class="ctrl w-time"
            />
          </div>
        ...
        <el-form-item :label="text('ikLab.fieldQuickRange', '快速套用')">
          <div class="quick-wrap">
            ...
            >
              {{ text(`ikLab.quickRanges.${item.key}`, item.fallback) }}
            </el-button>
          </div>
        </el-form-item>

        <el-form-item :label="text('ikLab.fieldPurpose', '用途')">
          <el-input
            ...
            maxlength="200"
            show-word-limit
            :placeholder="text('ikLab.placeholderPurpose', '請輸入用途說明')"
          />
        </el-form-item>

        <el-form-item :label="text('ikLab.fieldRemark', '備註')">
          <el-input
            v-model="form.remark"
            type="textarea"
            :rows="2"
            maxlength="150"
            show-word-limit
            :placeholder="text('ikLab.placeholderRemark', '可補充備註資訊')"
          />
        </el-form-item>
      </el-form>
...
function statusLabel (s) {
  ...
  if (v === 'approved') return text('ikLab.statusText.approved', '已核准')
  if (v === 'rejected') return text('ikLab.statusText.rejected', '已拒絕')
  if (v === 'canceled') return text('ikLab.statusText.canceled', '已取消')
  if (v === 'finished') return text('ikLab.statusText.finished', '已完成')
  return text('ikLab.statusText.pending', '待處理')
}
...
async function fetchSchedules () {
  ...
  try {
    ...
    console.error('❌ 取得 IK 排程失敗:', err)
    schedules.value = []
    ElMessage.error(err?.response?.data?.message || text('ikLab.message.loadFailed', '載入排程失敗'))
  } finally {
    loading.value = false
  }
}
...
async function saveSchedule () {
  if (!form.date) return ElMessage.warning(text('ikLab.message.needDate', '請選擇日期'))
  if (!form.startTime || !form.endTime) return ElMessage.warning(text('ikLab.message.needTimeRange', '請選擇時間範圍'))
  if (form.endTime <= form.startTime) return ElMessage.warning(text('ikLab.message.timeInvalid', '結束時間必須晚於開始時間'))
  if (!String(form.purpose || '').trim()) return ElMessage.warning(text('ikLab.message.needPurpose', '請輸入用途'))
  ...
  try {
    ...
    ElMessage.success(data?.message || text('ikLab.message.createSuccess', '建立成功'))
    dialogVisible.value = false
    selectedDate.value = form.date
    await fetchSchedules()
  } catch (err) {
    console.error('❌ 新增 IK 排程失敗:', err)
    ElMessage.error(err?.response?.data?.message || text('ikLab.message.createFailed', '建立失敗'))
  } finally {
    saving.value = false
  }
}
